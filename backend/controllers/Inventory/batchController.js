...
const createBatch = async (req, res, next) => {
  try {
    const { inventory_item_id, quantity, expiry_date, supplier, funding_source, cost, notes } = req.body
    ...
    if (!inventory_item_id || !quantity) throw new BadRequestError("Required fields are missing")
...
const getAllBatches = async (req, res, next) => {
  try {
    const { item_id, supplier, expiring_soon, is_active, search, page = 1, limit = 10 } = req.query
    ...
    if (expiring_soon === "true") {
      ...
      whereClause.expiry_date = {
        [Op.and]: [{ [Op.not]: null }, { [Op.lte]: thirtyDaysFromNow }, { [Op.gt]: new Date() }],
      }
    }
    if (search) {
...
const updateBatch = async (req, res, next) => {
  try {
    ...
    const { quantity, expiry_date, supplier, funding_source, cost, notes, is_active } = req.body
...
const getExpiringBatches = async (req, res, next) => {
  try {
    ...
    const { count, rows: batches } = await Batch.findAndCountAll({
      where: {
        expiry_date: {
          [Op.and]: [{ [Op.not]: null }, { [Op.lte]: expiryDate }, { [Op.gt]: new Date() }],
        },
        is_active: true,
      },
      include: [
