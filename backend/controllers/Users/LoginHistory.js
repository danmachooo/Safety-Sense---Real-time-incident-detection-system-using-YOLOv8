...
const getLoginHistory = async (req, res, next) => {
    try {
        ...
        // ✅ Sorting
        const validSortColumns = ["login", "logout", "firstname", "lastname", "email", "role"];
        const validSortOrders = ["asc", "desc"];

        let order = [["login", "desc"]];

        if (sortBy && validSortColumns.includes(sortBy)) {
            const direction = validSortOrders.includes(sortOrder) ? sortOrder : "desc";
...
module.exports = {
    logUserLogin, logUserLogout, getLoginHistory
}