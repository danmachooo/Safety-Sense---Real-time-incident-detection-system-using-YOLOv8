...
const changeUserRole = async (req, res, next) => {
    try {
        ...
        if(updatedUser) await sendRoleChangeEmail(updatedUser.email, updatedUser.role);

        if (!updatedUser || updatedUser.role !== role)  throw new Error('An error occurred. Cannot update user.');
...
const changeAccess = async (req, res, next) => {
    try {
        ...
        if(updatedUser) await sendAccountStatusEmail(updatedUser.email, updatedUser.isBlocked);

        await logAccountAction(
            adminId,
            id,
            isBlocked ? "BLOCK" : "UNBLOCK",
            `${user.firstname}'s account status has been changed to ${isBlocked ? "BLOCKED" : "UNBLOCKED"} by Admin ${admin.firstname}`
        );
