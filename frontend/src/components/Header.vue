...
<template>
  <header class="bg-[#1B262C] px-6  py-4 rounded-t-lg border-b flex items-center justify-between sticky top-0 z-10">
          ...
          <div class="p-4 border-b border-gray-200">
            <div class="flex justify-between items-center mb-4">
              <div class="flex items-center space-x-2">
                ...
                <span class="text-sm text-gray-500">({{ totalNotifications }})</span>
              </div>
              <div class="flex items-center space-x-2">
                <button 
                  @click="toggleSort"
            ...
            <div class="flex space-x-1 bg-gray-100 p-1 rounded-lg">
              <button
                v-for="tab in [{ id: 'unread', name: 'Unread' }, { id: 'read', name: 'Read' }]"
                :key="tab.id"
                @click="currentTab = tab.id"
                class="flex-1 px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 relative"
                :class="currentTab === tab.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'"
              >
                {{ tab.name }}
                <span 
            ...
            <template v-else-if="notifications.length > 0">
              ...
              >
                <div class="flex justify-between items-start">
                  <div class="flex-1">
                    <p class="text-sm text-gray-800">{{ notification.description }}</p>
